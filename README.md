...
```
Edit the `data/config.toml` file located in the project's directory with your telegram user id (you can obtain it using [myidbot](https://t.me/myidbot)) and the bot token generated by [BotFather](https://t.me/BotFather). To configure the items to track check the examples in the `config.toml` file. All fields are optional except for `keyword`.
